defmodule Day03.Part1 do
  ...
  def find_num({line, line_index}, symbol_map) do
    ...
    |> List.flatten()
    |> Enum.filter(fn {start_index, len} ->
      is_valid(symbol_map, line_index, start_index, start_index + len - 1)
    end)
  ...
  def is_valid(symbol_map, line_index, start_index, end_index) do
    ...
    if line_index == 0 do
      ...
      Enum.at(symbol_map, line_index - 1) |> Enum.any?(&(&1 in ext_rng))
    end ||
      if line_index + 1 >= Enum.count(symbol_map) do
        false
      else
        Enum.at(symbol_map, line_index + 1) |> Enum.any?(&(&1 in ext_rng))
      end ||
  ...
  def valid_nums(number_map, line_index, gear_index, lines) do
    ...
    nums_above =
      if line_index == 0 do
        ...
        |> Enum.filter(fn {start, len} ->
          gear_index >= start - 1 and gear_index <= start + len
        end)
        |> Enum.map(fn {start, len} ->
          lines |> Enum.at(line_index - 1) |> parse_number(start, len)
        end)
      end

    nums_below =
      if line_index + 1 >= Enum.count(number_map) do
        []
      else
        number_map
        |> Enum.at(line_index + 1)
        |> Enum.filter(fn {start, len} ->
          gear_index + 1 >= start and gear_index - 1 <= start + len - 1
        end)
        |> Enum.map(fn {start, len} ->
          lines |> Enum.at(line_index + 1) |> parse_number(start, len)
        end)
    ...
    nums_before =
      if gear_index == 0 do
        ...
        |> Enum.map(fn {start, len} ->
          lines |> Enum.at(line_index) |> parse_number(start, len)
        end)
      end

    nums_after =
      if gear_index >= String.length(Enum.at(lines, line_index)) do
        []
      else
        curr_map_line
    ...
    if Enum.count(nums) == 2 do
      Enum.reduce(nums, fn x, y -> x * y end)
    else
      0
    end
  end
